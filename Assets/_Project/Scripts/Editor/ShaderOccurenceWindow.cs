...
public class ShaderOccurenceWindow : EditorWindow
{
	...
	void OnGUI()
	{
		...
		List<Object> unityMaterials = new List<Object>();
		foreach(var pathMaterial in materials)
		{
			Object unityMaterial = AssetDatabase.LoadAssetAtPath(pathMaterial, typeof(Object));
			if (unityMaterial.name.ToLower() != "lit")
				unityMaterials.Add(unityMaterial);
		}

		Selection.objects = unityMaterials.ToArray();
